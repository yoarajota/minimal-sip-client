...
| R-003 | No liveness detection on an established dialog: a dead far end leaves the call hanging until the application decides to Hangup (BYE then times out at 32 s) | S-002 | mitigated | The P5 fault-injection leg (E-006) kills the Asterisk container mid-call at the real seam: media stops, the BYE transaction times out with a 408 after the 64×T1 window and the client exits cleanly. Residual: an established call still has no proactive liveness signal — the *outcome* is now defined and tested; detection is still the application's choice of when to Hangup |
